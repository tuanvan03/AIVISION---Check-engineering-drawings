...
import { Eye, EyeOff, Mail, Lock, ArrowRight, Scan } from "lucide-react"
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Priority: backend redirect_url > return_url query param > role-based default
      const returnUrl = searchParams.get("return_url")
      const targetUrl = redirectUrl || returnUrl || "/analysis"
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-primary/10 via-accent/20 to-background relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 xl:px-20">
          ...
          <p className="text-lg text-muted-foreground leading-relaxed max-w-md">
            Tự động kiểm tra, đối chiếu tiêu chuẩn ISO/ASME và phát hiện lỗi trong vài giây. 
            Tiết kiệm hàng giờ làm việc thủ công.
          </p>
          
          <div className="mt-12 flex flex-wrap gap-4">
            {["ISO 129", "ASME Y14.5", "GD&T", "Auto CAD"].map((tag) => (
              <span
                key={tag}
                className="px-4 py-2 rounded-full bg-background/80 backdrop-blur-sm border border-border/50 text-sm font-medium shadow-sm"
              >
                {tag}
              </span>
            ))}
